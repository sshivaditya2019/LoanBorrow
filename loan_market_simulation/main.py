...
def plot_results(history):
    ...
    ax7 = axs[3, 0]
    gdp = np.array(history['gdp_growth'])
    ax7.plot(time_steps, gdp, label='GDP Growth', color='red', alpha=0.5)
    ax7.set_title('GDP Growth vs Time Step')
    ax7.set_xlabel('Time Step')
    ax7.set_ylabel('GDP Growth', color='red')
    ax7.grid(True)

    ax8 = axs[3, 1]
    cci = np.array(history['cci'])
    ax8.plot(time_steps, cci, label='CCI', color='red', alpha=0.5)
    ax8.set_title('CCI vs Time Step')
    ax8.set_xlabel('Time Step')
    ax8.set_ylabel('CCI', color='red')
    ax8.grid(True)
    ...
    for episode in range(num_episodes):
        ...
        while not done and time_step < max_time_steps:
            ...
            for key in ['avg_credit_score', 'avg_income', 'avg_debt', 'num_loans', 
                       'avg_interest_rate', 'economic_cycle', 'gdp_growth', 'cci']:
