...
class LoanMarketEnvironment:
    def __init__(self, lenders, borrowers, best_values=None, credit_length=None):
    ...
    def get_state(self):
        if self.credit_length:
            return {
                ...
                'avg_credit_len': np.mean([b.credit_length for b in self.borrowers]),
    ...
    def get_market_liquidity(self):
        ...
        total_borrower_debt_capacity = sum(
            max(0, borrower.income * 0.5 - borrower.debt) for borrower in self.borrowers)
    ...
    def step(self):
        ...
        for lender in self.lenders:
            if lender.capital > lender.min_loan_amount:  # Only make offers if the lender has enough capital
                ...
                # Add the loan offer to the list
                loan_offers.append(
                    (lender, (interest_rate, loan_amount, term)))
        ...
        for borrower in self.borrowers:
            if borrower.can_borrow():  # Only evaluate loan offers if the borrower can borrow
                for lender, offer in loan_offers:
                    ...
                    loan = Loan(lender, borrower, loan_amount,
                                interest_rate, term)  # Create a loan object

                    # Check if the lender can grant the loan
                    if lender.assess_loan(loan, borrower):
                        decision = borrower.evaluate_loan(
                            loan, self.state)  # Evaluate the loan offer

                        if decision:
                            # Apply for the loan and grant it
                            if borrower.apply_for_loan(loan) and lender.grant_loan(loan):
                                ...
                                # Update rewards for the borrower
                                borrower_rewards[borrower.id] += loan_amount
                                # Update rewards for the lender
                                lender_rewards[lender.id] -= loan_amount
                                print(
                                    f"Loan created: Lender {lender.id}, Borrower {borrower.id}, Amount: {loan_amount:.2f}, Interest: {interest_rate:.2%}, Term: {term}")
        ...
        for loan in self.loans[:]:
            if loan.make_payment():
                ...
                print(
                    f"Loan defaulted: Lender {loan.lender.id}, Borrower {loan.borrower.id}, Amount: {loan.amount:.2f}")

        # Update states and policies
        next_state = self.get_state()
        for lender in self.lenders:
            lender.update_state(self.state, lender.get_action(
                self.state), lender_rewards[lender.id], next_state)
        for borrower in self.borrowers:
            ...
            action = borrower.evaluate_loan(
                Loan(None, borrower, 0, self.min_interest_rate, 12), self.state)
            borrower.update_state(
                self.state, action, borrower_rewards[borrower.id], next_state, dummy_loan_offer)

        self.state = next_state

        # Stop when the simulation reaches the maximum time step or when there are no more loans or capital
        done = self.time_step >= 720 or len(self.loans) == 0 and all(
            lender.capital <= lender.min_loan_amount for lender in self.lenders)
    ...
    def print_statistics(self):
        if self.credit_length:
            ...
            print(
                f"Economic cycle: {'Boom' if self.economic_cycle == 1 else 'Recession' if self.economic_cycle == -1 else 'Neutral'}")
            print(
                f"Average credit score: {self.state['avg_credit_score']:.2f}")
            print(f"Average income: ${self.state['avg_income']:.2f}")
            print(f"Average debt: ${self.state['avg_debt']:.2f}")
            print(f"Average credit length: {self.state['avg_credit_len']}")
            print(f"Number of active loans: {self.state['num_loans']}")
            print(f"Default rate: {self.state['default_rate']:.2%}")
            print(
                f"Average interest rate: {self.state['avg_interest_rate']:.2%}")
            print(f"Market liquidity: {self.state['market_liquidity']:.2f}")
            print(
                f"Total lender capital: ${sum(lender.capital for lender in self.lenders):.2f}")
            print(
                f"Total borrower debt: ${sum(borrower.debt for borrower in self.borrowers):.2f}")
        else:
            print(f"Time step: {self.time_step}")
            print(
                f"Economic cycle: {'Boom' if self.economic_cycle == 1 else 'Recession' if self.economic_cycle == -1 else 'Neutral'}")
